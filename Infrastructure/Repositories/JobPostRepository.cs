    ...
    public class JobPostRepository : GenericRepository<JobPost>, IJobPostRepository
    {
        ...
        public async Task<List<JobPost>> SearchJobPosts(SearchJobPostRequest request)
        {
            try
            {
                ...
                // If no filters were applied, add a default condition to retrieve all records
                if (predicate.Body == null || predicate.Body.NodeType == ExpressionType.Constant)
                {
                    predicate = PredicateBuilder.New<JobPost>(true); // Always true to get all records
                }
        ...
        public async Task<int> CountTotalPaging(SearchJobPostRequest request)
        {
            try
            {
                ...
                if (predicate.Body == null || predicate.Body.NodeType == ExpressionType.Constant)
                {
                    predicate = PredicateBuilder.New<JobPost>(true);
                }
                query = query.Where(predicate);
